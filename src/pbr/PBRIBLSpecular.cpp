...
void PBRIBLSpecular::init(int w, int h) {
    SCR_WIDTH = w;
    SCR_HEIGHT = h;
...
std::shared_ptr<SKShader> PBRIBLSpecular::createSKShader(std::string vsFile, std::string fsFile, std::string gsFile){
    char* gsBuf = nullptr;
    if(gsFile != "resource/"){
...
void PBRIBLSpecular::loadShader(){
	/*create shaders
	 */
	std::string fileList[][3] = {
        {"shaders/pbr_ibl_specular.vs", "shaders/pbr_ibl_specular.fs", ""},
        {"shaders/cubemap.vs", "shaders/equirectangular_to_cubemap.fs", ""},
        {"shaders/cubemap.vs", "shaders/irradiance_convolution.fs", ""},
        {"shaders/cubemap.vs", "shaders/prefilter.fs", ""},
        {"shaders/brdf.vs", "shaders/brdf.fs", ""},
        {"shaders/background.vs", "shaders/background.fs", ""},
	};
...
void PBRIBLSpecular::loadTexture()
{
 
}

unsigned int PBRIBLSpecular::loadTexture(char const * path_str, bool relativeToAbsolute)
{
...
void PBRIBLSpecular::updateUI(int w, int h) {
...
void PBRIBLSpecular::drawSprite(int w, int h, vec2f offset){

}

vec2f PBRIBLSpecular::ndcToScreen(vec2f ndcPosition){
	vec2f outPos;
	ndcPosition.y *= -1;
...
vec2f PBRIBLSpecular::screenToNdc(vec2f screenPosition){
	vec2f outPos;
...
void PBRIBLSpecular::onScroll(float dxScreen, float dyScreen){
	camera.RotateCameraByMouseMove(dxScreen, dyScreen, lookAt);
}

void PBRIBLSpecular::onFling(float vx, float vy){

}

void PBRIBLSpecular::run(float w, float h)
{
    static float dt = 0;
...
// --------------------
void PBRIBLSpecular::renderScene(SKShader &shader, float w, float h)
{
...
void PBRIBLSpecular::renderCube()
{
...
void PBRIBLSpecular::renderQuad(){
     if (quadVAO == 0)
    {
        float quadVertices[] = {
...
void PBRIBLSpecular::renderSphere(){
    if (sphereVAO == 0)
    {
...
void PBRIBLSpecular::processInput(GLFWwindow *window)
{
