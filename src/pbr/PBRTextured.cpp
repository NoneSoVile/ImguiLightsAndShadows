...
void PBRTextured::init(int w, int h) {
    SCR_WIDTH = w;
    SCR_HEIGHT = h;
...
std::shared_ptr<SKShader> PBRTextured::createSKShader(std::string vsFile, std::string fsFile, std::string gsFile){
    char* gsBuf = nullptr;
    if(gsFile != "resource/"){
...
void PBRTextured::loadVars() {
    camera = SKCamera(glm::vec3(0.0f, 0.0f, 5.0f));
    lookAt = glm::vec3(0.0f, 0.0f, 0.0f);
    cameraTranslation = glm::vec3(0.0f, 0.0f, 0.0f);

    albedo    = loadTexture("resource/images/pbr/rusted_iron/albedo.png", true);
    normal    = loadTexture("resource/images/pbr/rusted_iron/normal.png", true);
    metallic  = loadTexture("resource/images/pbr/rusted_iron/metallic.png", true);
    roughness = loadTexture("resource/images/pbr/rusted_iron/roughness.png", true);
    ao        = loadTexture("resource/images/pbr/rusted_iron/ao.png", true);
...
void PBRTextured::loadShader(){
	...
	for (size_t i = 0; i < ARRAY_SIZE(fileList); i++)
	{
		...
		BIND_SHADER(shader, 0, shaderPbrt, i);
    ...
    shaderPbrt->use();
    shaderPbrt->setMat4("projection", projection);
}

void PBRTextured::loadMesh()
{
    
}
float ourLerp1(float a, float b, float f)
{
    return a + f * (b - a);
}

#define BIND_TEXTURE(texID, index, nameTexture, i) if(i == index){nameTexture = texID;}
void PBRTextured::loadTexture()
{
 
}

unsigned int PBRTextured::loadTexture(char const * path_str, bool relativeToAbsolute)
{
...
void PBRTextured::updateUI(int w, int h) {
...
void PBRTextured::drawSprite(int w, int h, vec2f offset){

}

vec2f PBRTextured::ndcToScreen(vec2f ndcPosition){
	vec2f outPos;
	ndcPosition.y *= -1;
...
vec2f PBRTextured::screenToNdc(vec2f screenPosition){
	vec2f outPos;
...
void PBRTextured::onScroll(float dxScreen, float dyScreen){
	camera.RotateCameraByMouseMove(dxScreen, dyScreen, lookAt);
}

void PBRTextured::onFling(float vx, float vy){

}

void PBRTextured::run(float w, float h)
{
    static float dt = 0;
    ...
    shaderPbrt->setMat4("view", view);
    shaderPbrt->setVec3("camPos", camera.Position);
...
// --------------------
void PBRTextured::renderScene(SKShader &shader, float w, float h)
{
...
void PBRTextured::processInput(GLFWwindow *window)
{
