...
void ShadowMappingBase::init(int w, int h) {
...
std::shared_ptr<SKShader> ShadowMappingBase::createSKShader(std::string vsFile, std::string fsFile){
...
void ShadowMappingBase::loadVars() {
...
void ShadowMappingBase::loadShader(){
	/*create shaders
	 */
	std::string fileList[][2] = {
        ...
        {"shaders/shadow_mapping.vs", "shaders/shadow_mapping.fs"},
	};
...
void ShadowMappingBase::loadMesh()
{
...
void ShadowMappingBase::loadTexture()
{
...
void ShadowMappingBase::updateUI(int w, int h) {
...
void ShadowMappingBase::drawSprite(int w, int h, vec2f offset){

}

vec2f ShadowMappingBase::ndcToScreen(vec2f ndcPosition){
	vec2f outPos;
	ndcPosition.y *= -1;
...
vec2f ShadowMappingBase::screenToNdc(vec2f screenPosition){
	vec2f outPos;
...
void ShadowMappingBase::onScroll(float dxScreen, float dyScreen){
	camera.RotateCameraByMouseMove(dxScreen, dyScreen, modelTranslation);
}

void ShadowMappingBase::onFling(float vx, float vy){

}

void ShadowMappingBase::run(float w, float h) {
	static float dt = 0;
    ...
    normalSceneShader->setMat4("lightSpaceMatrix", lightSpaceMatrix);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, woodTexture);
...
// --------------------
void ShadowMappingBase::renderScene(const SKShader &shader)
{
    // floor
